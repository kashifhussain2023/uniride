...
export default function Review({ endRideData, userAuth, acceptDriverDetail }) {
  ...
  const handleRatingChange = (index, rating) => {
    ...
    newRatings[index] = { id: reviewQuestionDetail[index]?.id, rating };
    setRatings(newRatings);
  };
  const handleTipClick = (tipPercentage, index) => {
    const tipAmount = (totalAmount * tipPercentage) / 100;
    let roundedNumber = tipAmount.toFixed(1);
  ...
  const submitReview = async () => {
    let filteredRating = ratings.filter((element) => {
      ...
      <PannelSection>
        <LeftPannel>
          ...
          <TipYourDriver>
            ...
            <Typography variant="subtitle3" component="p">
              Thank your driver by leaving them a Tip. 100% of the Tip will go
              to your driver.
            </Typography>
            <List>
              <ListItem>
            ...
            <RouteDetail>
              <List>
                ...
                <ListItem>
                  ...
                  <Details>
                    {rideResponse.payment_type == 1 ? "Card" : ""}
                  </Details>
                </ListItem>
                <ListItem>
