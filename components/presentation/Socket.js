...
class SocketService {
  ...
  emit(event, data) {
    ...
    this.logEvent('emit', event, eventData);
  ...
  clearEventHistory() {
    this.eventHistory = [];
    debugLog('Event history cleared');
  }

  // Clear connection history
  clearConnectionHistory() {
    this.connectionHistory = [];
    debugLog('Connection history cleared');
  }
...
const socketHelpers = {
  ...
  requestRideCompleteness: (customerId, tokenCode) => {
    debugLog('Requesting ride completeness', {
      customerId,
      tokenCode,
    });
    return new Promise((resolve, reject) => {
  ...
  requestScheduleRideDetails: (customerId, tokenCode) => {
    debugLog('Requesting schedule ride details', {
      customerId,
      tokenCode,
    });
    return new Promise((resolve, reject) => {
  ...
  requestSendBooking: (payload) => {
    ...
    return new Promise((resolve) => {
      ...
      // 30 seconds timeout to match server timeout
