...
export async function middleware(request) {
    ...
    // Special handling for add-card page
    if (pathname === '/add-card') {
        // Check for new user registration cookie
        const newUserCookie = request.cookies.get('newUserRegistration');
        if (newUserCookie) {
            return NextResponse.next();
        }

        // If no registration cookie and no token, redirect to login
        if (!token) {
            return NextResponse.redirect(new URL(getLoginPath(), request.url));
        }
    }
